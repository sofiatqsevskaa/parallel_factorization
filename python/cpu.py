...
def pollards_rho(n):
    ...
    return d if d <= sqrt_n else n // d


def parallel_pollards_rho(n, num_attempts=5, timeout=60):

    with multiprocessing.Pool(processes=num_attempts) as pool:
        results = [pool.apply_async(pollards_rho, (n,))
                   for _ in range(num_attempts)]

        for result in results:
            try:
                factor = result.get(timeout=timeout)
                if factor:
                    pool.terminate()
                    return factor
...
def factorize_from_file(input_filename, output_filename, num_attempts=5):
    with open(input_filename, "r") as infile, open(output_filename, "w") as outfile:
        ...
        for line in infile:
            ...
            start = time.time()
            factor = parallel_pollards_rho(n, num_attempts)
            end = time.time()
...
if __name__ == "__main__":
    factorize_from_file("input.txt", "golemiot_cpu.txt")
